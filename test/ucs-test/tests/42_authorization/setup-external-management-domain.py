...
def create_idp_in_ucs_keycloak():
     ...
     * hardcoded mapper to store nubus_external_account=true
    """
    ...
    mappers = [
        {
            'name': 'nubus_external_account',
            'identityProviderAlias': IDP_ALIAS,
            'identityProviderMapper': 'hardcoded-attribute-idp-mapper',
            'config': {'attribute.value': 'true', 'syncMode': 'INHERIT', 'attribute': 'nubus_external_account'},
        },
        {
            'name': 'nubus_roles',
            'identityProviderAlias': IDP_ALIAS,
    ...
    mappers = [
        ...
        },
        {
            ...
            'config': {
                'introspection.token.claim': 'true',
                'userinfo.token.claim': 'true',
                'user.attribute': 'nubus_external_account',
                'id.token.claim': 'true',
                'lightweight.claim': 'false',
                'access.token.claim': 'true',
                'claim.name': 'nubus_external_account',
...
def create_user_profile():
    ...
    profile['attributes'].append(
        {
            'name': 'nubus_external_account',
            'displayName': '${profile.attributes.nubus_external_account}',
            'permissions': {'edit': ['admin'], 'view': ['admin']},
            'multivalued': False,
            'group': 'user-metadata',
        },
...
def create_external_accounts_container():
    ...
    position.setDn(f'cn=groups,{ldap_base}')
    group = groups.object(None, lo, position)
    group['name'] = 'external_accounts'
    group_dn = group.create()
    position.setDn(f'{ldap_base}')
    cn = cns.object(None, lo, position)
    cn['name'] = 'external_accounts'
    cn.options.append('group-settings')
    cn['defaultGroup'] = group_dn
    cn.create()
...
def main():
    ...
    create_user_profile()
    create_external_accounts_container()
