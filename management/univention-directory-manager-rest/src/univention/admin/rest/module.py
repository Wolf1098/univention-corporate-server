...
class Objects(ConditionalResource, FormBase, ReportingBase, _OpenAPIBase, Resource):
    ...
    async def get(
        ...
        if search:  # TODO: check if searching is allowed
            try:
                objects, length, last_page = await self.search(module, position, ldap_filter, superordinate, scope, hidden, items_per_page, page, by or module.default_search_property, reverse, opened)
