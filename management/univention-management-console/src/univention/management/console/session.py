...
class User:
    ...
    __slots__ = ('_locale', 'auth_type', 'authenticated', 'external_account', 'ip', 'object_id', 'password', 'roles', 'session_end_time', 'user_dn', 'username')
...
class Session:
    ...
    def set_credentials(self, username, password, auth_type, object_id=None, roles=None, external_account=False):
        self.user.authenticated = True
        self.user.username = username
        self.user.object_id = object_id
        self.user.auth_type = auth_type
        self.user.external_account = external_account
    ...
    def _search_user_dn(self):
        ...
        if self.user.external_account:
            username = self.user.object_id
            s_filter = filter_format('(univentionObjectIdentifier=%s)', (self.user.object_id,))
        else:
        ...
        if lo and username:
            ...
            try:
                ...
                ldap_dn = None
                CORE.error('Could not get uid for %r: %s' % (username, traceback.format_exc()))
            if ldap_dn:
...
class IACLs:
    ...
    def _get_acls(self):
        ...
        if not sess.user.authenticated:
            ...
            if sess.user.external_account:
                # FIXME: return UMC_ACLS_FROM GUARDIAN
                pass
            # FIXME: just for testing
            return LDAP_ACLs('Administrator', 'uid=Administrator,cn=users,dc=ucs,dc=test', ucr['ldap/base'])
